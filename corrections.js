...
function receipt(paytype) {
    switch (paytype.payment_type){
        case "creditcard":
...
if (order.payment_type != "free"){
    isNotFree = (<p>{receipt(order.payment_type)} was charged ${order.amount_in_dollars}</p>)
}
    
html = function (){
    return (
        <div>
        <h1> Order receipt details </h1>
        <p>Your order of {order.products.name} has been received.</p> {ifNotFree}
        </div>
        )}
